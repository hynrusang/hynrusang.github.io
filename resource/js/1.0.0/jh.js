...
class DomDefault {
     ...
     */
    children = num => {
        if (!this._node.children[num]) throw `노드의 ${num + 1}번째 자식노드는 존재하지 않습니다.`;
        return this._node.children[num];
    }
    /**
    ...
    add = (...dom) => {
        for (let i = 0; i < dom.length; i++) {
            if (Array.isArray(dom[i])) {
                for (let j = 0; j < dom[i].length; j++) this._node.appendChild(dom[i][j]._node);
            } else this._node.appendChild(dom[i]._node);
        }
        return this;
    }
    /**
