    ...
    Youtube: {
        ...
        Items: (dataset, key) => Object.keys(dataset[key]).sort().map((data, index) => {
            let field = $("a", {
                ...
                onclick: e => {
                    e.preventDefault();
                    const href = e.target.href.includes("list=") ? `${e.target.href.replace("m.", "www.").replace("playlist", "embed/videoseries/").replace("watch", "embed/videoseries/")}&amp;loop=1&autoplay=1` : `https://www youtube.com/embed/${dataset[key][data].match(/[?&]v=([^&]+)/)[1]}`;
